...
class OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        return self.async_show_form(
            step_id="init", data_schema=data_schema, errors=self._get_form_error(),
        )
        ...
        config_schema = vol.Schema(
            {
                ...
                vol.Optional(
                    CONF_MIN_KELVIN, default=curr_conf.get(CONF_MIN_KELVIN, min_kelvin),
                ): vol.All(vol.Coerce(int), vol.Clamp(min=min_kelvin, max=max_kelvin)),
                vol.Optional(
                    CONF_MAX_KELVIN, default=curr_conf.get(CONF_MAX_KELVIN, max_kelvin),
                ): vol.All(vol.Coerce(int), vol.Clamp(min=min_kelvin, max=max_kelvin)),
                vol.Optional(
                    CONF_TUYA_MAX_COLTEMP,
